...
#go=msigdb.gsea(sigmod,background = "annotation",method="hypergeometric",species = "human")
...
library(DGCA,lib.loc='../../R')
library(dplyr)
 design_mat=model.matrix(~0+meta$Group)
colnames(design_mat)=c("Cannabis","Control")
moduleDC_res=moduleDC(inputMat = expr,design = design_mat,
                      compare = c("Cannabis","Control"),genes=sigmod$values,
                      labels=sigmod$ind,nPerm=500, gene_avg_signif = 1,
                      number_DC_genes = 10)
#moduleDC_res=moduleDC(inputMat = expr,design = design_mat,compare = c("Control","Icer"),genes=sigmod$values,labels=sigmod$ind,nPerm=50)
